...
run_fits <- function(rep_plot,nn,cut_off,dt,filename="1"){
  ...
  for(kk in 1:rep_plot){
    ...
    if(output_smc$lik != - Inf){
      ...
      # rpois for the interquantile range
...
plot_outputs <- function(rep_plot, filename="1"){
  ...
  C_local_plot_raw = C_local_plot_raw[,!is.na(C_local_plot_raw[t_period-1,])] # output raw cases
  ...
  # Local cases
  Case_local_quantile <- apply(C_local_plot,1,function(x){quantile(x,c(0.025,0.25,0.5,0.75,0.975), na.rm = TRUE)}) 
  Case_local_quantile_onset <- theta[["onset_prop"]]*Case_local_quantile
  
  # DEBUG - output data 
  # 
  Case_local_quantile_raw <- apply(C_local_plot_raw,1,function(x){quantile(x,c(0.025,0.25,0.5,0.75,0.975), na.rm = TRUE)}) 
  ...
  Rep_local_quantile <- apply(Rep_local_plot,1,function(x){quantile(x,c(0.025,0.25,0.5,0.75,0.975), na.rm=T)}) 
  ...
  ym1 <- 25
  plot(date_rangeA,Case_local_quantile_onsetA[1,],col="white",ylim=c(0,ym1),xlim=c(xMin1,xMax),xlab="",ylab="Daily new cases in China")
  polygon(c(date_rangeF,rev(date_rangeF)),c(yyF,rev(yyF+1e5)),lty=0,col=rgb(0.9,0.9,0.9))
  ...
  legend(date_rangeA[23], 25, legend=c("Data", "Kucharski (2020)", "50% credible interval", "95% credible interval"), 
  ...
  ym1 <- 10
  plot(date_range,case_time,pch=19,ylim=c(0,ym1),xlim=c(xMin1,xMax),ylab="Daily cases internationally",col="white")
  polygon(c(date_rangeF,rev(date_rangeF)),c(yyF,rev(yyF+1e5)),lty=0,col=rgb(0.9,0.9,0.9))
  ...
  legend(date_rangeA[23], 10, legend=c("Data", "Kucharski (2020)", "50% credible interval", "95% credible interval"), 
         box.lwd = 1, box.col = "grey",bg = "white",
         col=c("black", "blue", rgb(0,0.3,1,0.65), rgb(0,0.3,1,0.2)), lty= c(NA, 1, 1, 1), lwd = c(NA, 2, 8, 10), pch=c(19, NA, NA, NA),
         seg.len=0.25, y.intersp=0.65, x.intersp=0.25, cex=0.8,
         text.width = 15)
  ...
  ym1 <- 50000
  plot(date_range,Rep_local_quantile[1,],col="white",ylim=c(0,ym1),xlim=c(xMin1,xMax),xlab="",ylab="Daily new cases in Wuhan")
  ...
  legend(date_rangeA[23], 5000, legend=c("Data (out-of-sample)", "Kurchaski (2020)", "50% credible interval", "95% credible interval"), 
         box.lwd = 1, box.col = "grey",bg = "white",
         col=c("black", "blue", rgb(0,0.3,1,0.65), rgb(0,0.3,1,0.2)), lty= c(NA, 1, 1, 1), lwd = c(NA, 2, 8, 10), pch=c(19, NA, NA, NA),
         seg.len=0.25, y.intersp=0.65, x.intersp=0.25, cex=0.8,
         text.width = 15)  
  ...
  title(ylab="Daily new cases internationally", line=1.5, cex.lab=1)
  
  legend(date_rangeA[23], 10, legend=c("Data (out-of-sample)", "Posterior mean", "50% credible interval", "95% credible interval"), 
         box.lwd = 1, box.col = "grey",bg = "white",
         col=c("black", "blue", rgb(0,0.3,1,0.65), rgb(0,0.3,1,0.2)), lty= c(NA, 1, 1, 1), lwd = c(NA, 2, 8, 10), pch=c(19, NA, NA, NA),
         seg.len=0.25, y.intersp=0.65, x.intersp=0.25, cex=0.8,
         text.width = 15) 
