...
process_model <- function(t_start,t_end,dt,theta,simTab,simzetaA,travelF){
  ...
  for(ii in seq((t_start+dt),t_end,dt) ){
    ...
    # this is what they call in the supp mat Q_w, D_w, C_w
...
smc_model <- function(theta,nn,dt=1){
  ...
  # t_period is the epidemic duration set to 82
  ttotal <- t_period
  t_length <- ttotal
  ...
  for(tt in 2:ttotal){
    ...
    # run process model
    # storeL is the Euler step to get the next step simulation
    # can substite with our one step filtering
    storeL[,tt,] <- process_model(tt-1,tt,dt,theta,storeL[,tt-1,],simzeta[tt,],travelF)

    # calculate weights
    # use our predictive distribution
  ...
  return(list(ESS = ESS, S_trace=S_traj,C_local_trace=C_local_traj,Rep_local_trace=Rep_local_traj,C_trace=C_traj,Rep_trace=Rep_traj,E_trace=E_traj,I_trace=I_traj,beta_trace=beta_traj,lik=likelihood0 ))
...
AssignWeights <- function(data_list,storeL,nn,theta,tt){
  ...
  #  c_local_val are the new infected at time tt
  ...
  if(!is.na(local_case_data_tt)){
    # c_local_val are the new local infected at time tt in storeL (simulated)
    # theta[["confirmed_prop"]] is the proportion of confirmed reported 
    # theta[["onset_prop"]] is the proportion onsets known
    # theta[["local_rep_prop"]] is the local proportion reported
    # local_case_data_tt_scale
    # This compute the likelihood of the total new cases in China (Wuhan and China)
    # we will not have c_local_val but we have the expected proportion of as:
    #  theta[["confirmed_prop"]]*theta[["onset_prop"]]*theta[["local_rep_prop"]]*local_case_data_tt_scale
    # we could use as Nick said a Multinomial according to our predictive distribution
  ...
  # rep val are the simulated reported cases internationally
  #x_scaled <- theta[["confirmed_prop"]]*rep_val
  # top_risk is a table of risks connected to countries
  # this operation creat a proportion of infected proportional to the countries
  ...
  # nn is the number of particles
  # rep_data_tt are the confirmed international cases
  #y_lam <- rep(rep_data_tt,nn); #dim(y_lam) <- NULL 

  # Calculate likelihood
  # use a poisson with mean given by confirmed cases internationally  
  # multiply by international risk
  ...
  # c_val are the new international infected at time tt in storeL (simulated)
  # theta[["confirmed_prop"]] is the proportion of confirmed reported 
  # theta[["onset_prop_int"]] is the proportion onsets known internationally
  # top_risk$risk is the international risk
  # This compute the likelihood of the international cases 
  # we will not have cl_val but we have the expected proportion of as:
  #  theta[["confirmed_prop"]]*theta[["onset_prop_int"]]*top_risk$risk
  # we could use as Nick said a Multinomial according to our predictive distribution
  # there are na's though
  if(!is.na(case_data_tt)){
    x_scaled <- c_val*theta[["confirmed_prop"]]*theta[["onset_prop_int"]]
    ...
    # y_lam are the international cases per country
    y_lam <- case_data_tt
  ...
  if(!is.na(flight_info_tt)){
    ...
    # inf_prev are all the simulated exposed and infected asyntomatic 
    # this number is divided by the number of total travellers
    prob_inf <- pmax(0,inf_prev/theta[["pop_travel"]]) # ensure >=0
    
    #print(c(flight_prop_tt[1],flight_prop_tt[2],prob_inf[1])) DEBUG
    # flight_prop_tt is the number of infected per evacuation flights
